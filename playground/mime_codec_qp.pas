...
begin
  if (Length(Line)>0) and (Line[Length(Line)] in [#9,#32]) then
    if Length(Line)>=76 then begin      { absolutly no room: just put offending char into next line: }
      ...
      Line:=Line+'='#13#10'='+HexStr(Ord(s),2);
    end else
...
function TQuotedPrintableEncodingStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
...
function TQuotedPrintableDecodingStream.Seek(Offset: Longint; Origin: Word): Longint;
var
  ipos: LongInt;
