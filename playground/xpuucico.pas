...
{$IFDEF OS2 }
     xpos2,
{$ENDIF }
     xpglobal,typeform,fileio,inout,resource,winxp,xpnetcall,ObjCOM,Modem;
...
begin
  NoCarrier:=(not IgnCD and not CommObj^.Carrier) or break;
end;
...
var b : byte;
begin
  if CommObj^.GetChar and (length(recs)<255) then begin
...
begin
  if ParDebug then begin
    ...
    forcecolor:=false;
    lwdt:=50;
    wrt(mx,my+3,'Ã'+dup(lwdt,'Ä')+'´');
    wrt(mx,my+13,'Ã'+dup(lwdt,'Ä')+'´');
...
begin
  ...
  CommObj^.SendString(LeftStr(strs(transdata.filesize)+dup(20,#0),20),false);   { Laenge senden }
...
begin
  ...
  mdelay(500);
  flushinput(comnr);
  CommObj^.SendString(^P'S'+pointname+iifs(SizeN,' -N','')+#0,false);
  time(LoginTimeout);
  n:=5;
...
(* SetLanguage; *)
  ...
  exitproc:=@newexit;
  multi3:=WriteOnline;
  uucico:=fuucico(starttime,ende,waittime,sendtime,rectime);
(* WriteResultFile; *)
(* CloseResource; resopen:=false; *)
end.
{
  $Log$
  Revision 1.1  2001/01/18 22:40:54  cl
  - to be adapted to ObjCOM - does not compile (yet).
