...
{$I xp6s.inc}
  ...
  var i,first : integer;
    ...
    begin
      with PTccmore(Cccm[n]) do begin
        mail:=(cpos('@',adr)>0);
        if mail then begin      // persînlich mail
  ...
  begin
    ...
    p1:=1;
    while (p1<=cc_anz) and (PTccmore(Cccm[p1]).server=s1) do inc(p1);    //Anzah der mail fÅr Server 1
    p2:=p1;
...
//          Cccm[i-1]:=cmr;
  ...
  begin
    ...
    while i<=cc_anz do begin
      ...
      while (i<=cc_anz) and (PTccmore(Cccm[i]).server=s) do begin  // alles Åber den gleichen Serve
  ...
  begin
    ...
    while i<=cc_anz do begin
      ...
      while (i<=cc_anz) and ((PTccmore(Cccm[i]).mail and not ntCrossPM(PTccmore(Cccm[i]).nt)) or
            //                persînliche mail     und  kein  PM Crosspostings im netz mîglich
            (not PTccmore(Cccm[i]).mail and not ntCrossAM(PTccmore(Cccm[i]).nt)) or
            //  keine persînliche mail  und  keine  AM-Crosspostings mîglich
