...
begin
  ...
  dbSeek(d,boiName,ustr(forcebox));
  if dbFound then
...
begin
  ...
  dbSeek (ubase, uiName, ustr (empfaenger));
  if dbFound then
    dbRead (ubase, 'pollbox', empfBox)
  else
  begin
    dbSeek (bbase, biBrett, ustr (empfaenger));
    if dbfound then
      dbRead (bbase, 'pollbox', empfBox)
  end;
  if box = empfBox then
  begin
    sameServer := true;
    for i := 1 to cc_anz do
      if uStr (ccm^[i].server) <> uStr (empfBox) then
        SameServer:=false;
  end;
end;
...
{
  $Log$
  Revision 1.39.2.65  2003/05/01 14:22:50  mk
  ...
  JG:- Fix fÅr Uralt-Bug: Das Eintragen eines KopienempfÑngers ("k" im
       ...
       sowie anschlie·endes Erzwingen des Versendens Åber eine gemeinsame
       Serverbox ("o" im Sendefenster) konnte zu abenteuerlichen Effekten
       fÅhren (Absturz, HÑnger, duplizierte Nachrichten). Variable "First"
  ...
  MY:- Versionsstrings korrigiert/geÑndert:
       - Snapshot-Versionsstrings werden jetzt nach dem Muster
         ...
         (zusÑtzliche Leerzeichen vor Beta-String und vor/nach "@").
       ...
       - Bei eingehenden Fido-Nachrichten wird in der MAILER-Zeile die
         Tearline jetzt nur noch dann mit " / " an die PID angehÑngt, wenn
         PID und Tearline nicht gleich sind und die PID keinen der Strings
         "crosspoint", "openxp", "xp2" oder "xp " enthÑlt.
  ...
  MY:- Syntax bei RFC-PrioritÑt (Sendefenster) gefixt
  ...
  JG+MY:- Undokumentierte Funktion <Alt-A> im Sendefenster (Absender
          Ñndern) Åbernahm den Absender nicht korrekt, wenn bei einer
          Fido-Nachricht mit <F2> ein anderer Fido-Absender ausgewÑhlt
          wurde.
