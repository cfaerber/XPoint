      ...
      if spezial then begin
        ...
        if n<0 then begin
          ...
          case p of
            ...
            10   : if netztyp in netsSupportingPGP then
                     n:=21;  { PGP }
  ...
  if pm then fidoto:=''
    ...
    case netztyp of
      ...
      if netztyp in (netsRFC+[nt_Magic, nt_Pronet, nt_ZConnect])then begin
        if (fidoto=brettalle) or (blankpos(fidoto)=0) then fidoto:='';
        end else
        fidoto:='';
    end;
  ...
  if not verteiler then begin
    ...
    if umlaute=0 then
      if netztyp=nt_Fido then
        ...
        if(netztyp in netsRFC)and(FileContainsUmlaut)then hdp.x_charset:='ISO-8859-1';
    if iso then
      hdp.charset:='ISO1';
...
{
  $Log$
  Revision 1.128  2001/09/10 15:58:03  ml
  - Kylix-compatibility (xpdefines written small)
  - removed div. hints and warnings

  Revision 1.127  2001/08/27 09:13:43  ma
  - changes in net type handling (1)

  Revision 1.126  2001/08/12 20:01:40  cl
