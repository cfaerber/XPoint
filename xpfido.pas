{   $Id: xpfido.pas,v 1.84 2004/01/18 19:54:59 mk Exp $
...
procedure GetReqFiles(const adr:string; var files:string);
...
uses  xpnt,xp2,xp3,xp4e, classes,xpconst,
{$IFDEF Kylix}
  xplinux,
{$ELSE}
{$IFDEF Unix}
...
begin
  ...
  mwrt(x+3, y+7, GetRes2( 2100, 3));     { ~ÚÄÄÄ¯ [                    ] }
...
var t1: text;
    ...
    crash   : boolean;
    TempFileName: String;
    NewReqFile: TStringList;
begin                             
  crash:=false;
  TempFileName := TempFile('');
  NewReqFile := TStringList.Create;
  try
    if FileExists(ReqDat) then
    begin
      assign(t1,ReqDat);
      reset(t1);
      while not eof(t1) do begin
    ...
    if crash or (files<>'') then
    begin
      NewReqFile.Add(adr);
      if crash then
        NewReqFile.Add(CrashID);
      if files<>'' then
        NewReqFile.Add(Files);
      NewReqFile.Add('');
    end;
    NewReqFile.SaveToFile(ReqDat);
  finally
    NewReqFile.Free;
  end;
end;
...
var fn     : string;
  ...
  function overwrite(const fn:string):boolean;
  begin
