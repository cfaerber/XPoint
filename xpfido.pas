...
begin       { FidoSeekfile:string;************************ }
  ...
  readmask(brk);                         { MO: Dialog anzeigen/ausfÅhren }
  enddialog;
  ...
  if icase then seek:=ustr(mid(fidolastseek,3)) {seek, enthÑlt nun den suchstring}
  ...
  writeln(t2^,' ',dup(len+length(seek),'Õ'));
  writeln(t2^,' ',getres2(2120,9),' "',mid(fidolastseek,3),'"');   { 'Dateisuche nach' .. }
  writeln(t2^,' ',dup(len+length(seek),'Õ'));
  writeln(t2^);
  assign(t1^,FileLists);
  reset(t1^);
  while not eof(t1^) do begin                  {noch EintrÑge in der cfg.datei}
    readln(t1^,s); s:=trim(s);
    p:=cpos('=',s);
    if (s<>'') and (s[1]<>'#') and (s[1]<>';') and (p>0) then begin
      ...
      if existf(t3^) then begin
        ...
        reset(t3^);
        apos:=0;                               { MO ZeilenzÑhler ungÅltigsetzen}
        beginSafe:=false;
        begin                                  { write header}
          s3:=left(s,p-1)+' ';
          ...
          begin
            ...
            begin
              ...
              if testBlock = true then begin
               ...
               apos:=0;  { setze ZeilenZÑhler auf Start wenn Block geschrieben
