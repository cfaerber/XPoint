    ...
    if art<>1 then fname:=''
      ...
      while i<=length(fname) do
        if pos(fname[i],' _^$~!#%-{}()@''`.'+range('A','Z')
          {$IFDEF UnixFS}+range('a','z'){$ENDIF}+'0123456789éôö')>0 then
          inc(i)
        else
          delete(fname,i,1);
...

{
  ...
  MY:- Anzeige der Stammbox-Adresse unterhalb der Menueleiste korrigiert
       ...
       - Realname wird in Klammern angezeigt (falls es sich um einen
         ...
         die GesamtlÑnge von Adresse und Realname groesser als 76 Zeichen
         ist;
