...
begin
  ...
  attrtxt(col.colsel2high);            { 'Zusatz-MenÅ' / 'Funktionstasten' }
  mwrt(x+1,y+1,forms(sp(5+length(txt))+getres2(240,3),73+length(txt)-2));
               { 'MenÅ           Programm                   $FILE      B W L A  Mem' }
  p:=1;
  modi:=false;
  repeat
    moff;
...
begin
  ...
  { else wrt(34,1,'P');        }
    ...
    at(colbretterinv);
    wrt(31,6,forms('  /Z-NETZ/RECHNER/IBM/HARDWARE       (gewÑhlt)',50));
    at(colbretterhi);
...
begin
  with col do begin
    ...
    at(colmsgshigh);
    wrt(31,7,'˛     262 04.11.91 R.SAUER@BIONIC.ZER      Re: Per');
    wrt(31,9,'˛     532 05.11.91 U.FLECKENSTEIN@MAX-002. Personu');
    at(colmsgsinv);
...
begin
  with col do begin
    ...
    wrt(35,8,' Userliste       ');
    wrt(34,10,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
...
begin
  with col do begin
    wwin(35,75,10,18,'Kalender u.Ñ.',colutility,colutility);
    moff;
...
begin
  with col do begin
    ...
    at(colhelpslQVW); write('gewÑhltem');
...
begin
  with col do begin
    ...
    wrt(57,11,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
    wrt(57,12,'≥ Suchen       ^QF ≥');
    wrt(57,13,'≥ Ersetzen     ^QA ≥');
    wrt(57,14,'≥ Weitersuchen  ^L ≥');
    wrt(57,15,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
    wrt(57,16,'≥ Umbruch aus   F3 ≥');
    wrt(57,17,'≥ Umbruch ein   F4 ≥');
    wrt(57,18,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
    wrt(57,19,'≥ Optionen     ^OR ≥');
    wrt(57,20,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
    wrt(57,21,'≥ MIME-Anhang   @A ≥');
    wrt(57,22,'√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥');
    wrt(57,23,'≥ Beenden      Esc ≥');
    wrt(57,24,'¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ');
    wshadow(58,77,5,25);
    at(coleditmenuhi);
...
begin
  with col do begin
    ...
    wrt(31,17,hbar(48));
    wrt(69,7,'00:00:12');
    wrt(33,9,'Senden'); wrt(33,11,'Dateigrî·e');
    wrt(33,12,'Åbertragen'); wrt(33,14,'Restzeit');
    wrt(57,11,'Blockgrî·e'); wrt(57,12,'Durchsatz        cps');
    wrt(57,14,'Fehler');
    at(colmailerhi2);
...
begin
  if videotype=0 then
    rfehler(216)        { 'Bei Hercules-Grafikkarten nicht mîglich!' }
  else
  ...
  repeat
    ...
    case n of
       1 : repeat    { HauptmeÅ }
           ...
           until m=0;
   2,3,4 : begin     { UntermenÅ 1.-3. Ebene }
             showmenus0;
             ssp:=showmenus1;
             if n>2 then begin
               ssp; ssp:=showmenus2;
...
{
  $Log$
  Revision 1.12.2.7  2003/05/01 14:22:43  mk
