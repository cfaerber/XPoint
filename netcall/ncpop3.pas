{  $Id: ncpop3.pas,v 1.24 2004/07/26 17:22:08 mk Exp $
...
begin
  if (not FSupportUIDLs)or(not OnlyNew) then
    ...
    result:=Int64(Pointer(FAvailableUIDLs.Objects[Nr - 1]));
end;
...
begin
  ...
  if FSupportUIDLs then
    ...
    for Nr := 0 to FAvailableUIDLs.Count - 1 do
      if Int64(Pointer(FAvailableUIDLs.Objects[Nr])) = ID then
      begin
...
{
  $Log: ncpop3.pas,v $
  Revision 1.24  2004/07/26 17:22:08  mk
  - fixed RSET command (not used until now)

  Revision 1.23  2003/11/22 11:41:16  mk
  - added support for maximum mail size (pop3) and automatic dialing on win32

  Revision 1.22  2003/08/03 19:07:43  mk
