...
type
  ...
  public
    ...
    procedure OnCommBlocking(CommObj: TCommStream);
    procedure OnConnecting(CommObj: TCommStream; const Node, Serv: String);
    procedure OnConnected(CommObj: TCommStream; const Node, Serv: String);
  ...
  public
    ...
    property Timer: TTimer read FTimerObj;
...
begin
  inherited Create;
  FCommObj := nil;
  FTimerObj := TTimer.Create;;
end;

destructor TCommNetcall.Destroy;
begin
  FreeAndNil(FCommObj);
  FreeAndNil(FTimerObj);
  inherited Destroy;
end;
...
procedure TCommNetcall.OnCommBlocking(CommObj: TCommStream);
begin
  case WaitForObjects([FTimerObj,CommObj]) of
    -1: if not FGotUserBreak then
    ...
    0: raise ENetcallTimeout.Create('timeout');
  end;
...
procedure TCommNetcall.OnConnecting(CommObj: TCommStream; const Node, Serv: String);
begin
  Self.Output(mcVerbose,'Connecting to %s:%s...',[Node,Serv]);
end;

procedure TCommNetcall.OnConnected(CommObj: TCommStream; const Node, Serv: String);
var s,p: string;
begin
  Self.Output(mcInfo,'Connected to %s:%s.',[Node,Serv]);

  s := TRawIPStream(CommObj).RemoteNode;
  p := IntToStr(TRawIPStream(CommObj).RemotePort);

  if ProgressOutput is TProgressOutputWindow then
    TProgressOutputWindow(ProgressOutput).Headline :=
      TProgressOutputWindow(ProgressOutput).Headline +' ('+s+':'+p+')';

  Log('=','Remote Address: '+s+', Port: '+p);

  s := TRawIPStream(CommObj).LocalNode;
  p := StrS(TRawIPStream(CommObj).LocalPort);

  Log(' ','Local Address: '+s+', Port: '+p);
end;
...
procedure TCommNetcall.Disconnect;
begin
  FreeAndNil(FCommObj);  
end;
...
begin
  FreeAndNil(FCommObj);
  FCommObj := NewCommObj;
  FCommObj.OnBlocking := Self.OnCommBlocking;

  if FCommObj is TRawIPStream then begin
    TRawIPStream(FCommObj).OnConnecting := Self.OnConnecting;
    TRawIPStream(FCommObj).OnConnected := Self.OnConnected;
  end;
end;
