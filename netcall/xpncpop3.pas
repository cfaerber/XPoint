{  $Id: xpncpop3.pas,v 1.45 2004/01/17 16:33:53 mk Exp $
...
type TPOP3SpoolDir = class
public
  Directory: String;            { '' = temporary directory! }
  function CreateFile(const id: string):TStream;
end;

function TPOP3SpoolDir.CreateFile(const id: string):TStream;
var fn: string;
begin
  fn := GetUniqueDownloadName(UpperCase(id)+'.IN',Directory);
...
var
  ...
  UIDLFileName  : String;
  UIDDFileName  : String;
  SpoolDirObj   : TPOP3SpoolDir;  

  procedure ProcessIncomingFiles;
  var iFile: Integer;
    fn : string;
    OutSpool : TZCSpoolOut;
    InSpool : TRFCSpoolIn;
    PO: TProgressOutputWindow;
  begin
    fn := TempS(10000);
    ...
    PO := nil;
    try
      PO := TProgressOutputWindow.CreateWithSize(60,1,'',true);
      InSpool := TRFCSpoolIn.Create(SpoolDirObj.Directory+'*.IN');
      InSpool.ForceEnvelopeTo := BP^.pop3_EnvelopeTo;
      InSpool.ProgressOutput := PO;
      OutSpool := TZCSpoolOut.Create(fn);
      InSpool.CopyTo(OutSpool);
      DeleteSpoolFiles.AddStrings(Inspool.FileList);
    finally
      OutSpool.Free;
      InSpool.Free;
      PO.Free;
...
begin
  ...
  SpoolDirObj := TPOP3SpoolDir.Create;
  SpoolDirObj.Directory := IncludeTrailingPathDelimiter(ExpandFileName(XFerDir+'TEST'+'.SPL'));
  CreateMultipleDirectories(SpoolDirObj.Directory);
  ...
  POP.OnCreateSink := SpoolDirObj.CreateFile;
  ...
  if FileExists(UIDLFileName)then POP.DownloadedUIDLs.LoadFromFile(UIDLFileName);

  { Get UIDLs to be deleted from file }
  UIDDFileName:=FileUpperCase(OwnPath+GetServerFilename(Boxname, extUdd));
  if FileExists(UIDDFileName)then POP.DeleteableUIDLs.LoadFromFile(UIDDFileName);
  ...
  try
    ...
    if POP.DownloadedUIDLs.Count>0 then
      POP.DownloadedUIDLs.SaveToFile(UIDLFileName)
    else
      DeleteFile(UIDLFileName);

    if POP.DeleteableUIDLs.Count>0 then
      POP.DeleteableUIDLs.SaveToFile(UIDDFileName)
    else
      DeleteFile(UIDDFileName);
