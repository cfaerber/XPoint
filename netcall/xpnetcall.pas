{  $Id: xpnetcall.pas,v 1.82 2004/01/17 16:33:53 mk Exp $
...
function  netcall2(PerformDial: boolean; BoxName: string; DialOnlyOnce,Relogin,FidoCrash: boolean): boolean;
...
function GetUniqueDownloadName(s, path:string): string;
...
begin
  s := GetUniqueDownloadName(s,path);
...
begin                  { function Netcall }
  ...
  email := ComputeUserAddress(BoxBase);
  ppfile:=bfile+extBoxFile;
  eppfile:=bfile+extEBoxFile;
  dbRead(BoxBase,'netztyp',netztyp);

  Debug.DebugLog('xpnetcall','got net type: '+ntName(netztyp),DLInform);

  ReadBox(netztyp,bfile,BoxPar);               { Pollbox-Parameter einlesen }
  BoxPar^._domain   := dbReadStr(BoxBase,'domain');  // Warum nicht aus bfg??
  komment := dbReadStr(BoxBase,'kommentar');
  Debug.DebugLog('xpnetcall','got server file name: '+bfile,DLInform);

  if not(netztyp IN [nt_Fido,nt_ZConnect,nt_POP3,nt_NNTP, nt_IMAP, nt_UUCP,
    nt_Maus, nt_Client])then
  begin
...
//**    AppendEPP;
  ...
  begin
    ...
    case netztyp of
      ...
      nt_Maus: begin
        Debug.DebugLog('xpnetcall','netcall: uucp',DLInform);
        case MausTauschNetcall(BoxName,Boxpar,BFile,ppfile,boxpar^.sysopmode,NetcallLogfile,IncomingFiles,DeleteSpoolFiles) of
          EL_ok     : begin Netcall_connect:=true; result:=true; end;
          EL_noconn : begin Netcall_connect:=false; end;
          EL_recerr,
          EL_senderr,
...
begin
  ...
  if not ende then
    netcall2(true,BoxName,false,false,false);
...
begin
  ...
  b:=exclude_time;
  if b=0 then
    if netcall2(true,BoxName,false,false,false) then
    else
  ...
  if ParNetcall<>'' then
    if ParNSpecial then
      if ival(ParNetcall) in [1..20] then
        ...
        trfehler1(749,ParNetcall,60) { '/nsp: Ungltige Zeilenangabe fr NETCALL.DAT: %s' }
      ...
      if ParNCtime='' then
        Netcall2(true,ParNetcall,false,ParRelogin,false)
      else
...
{
  $Log: xpnetcall.pas,v $
  Revision 1.82  2004/01/17 16:33:53  mk
  - split xp0.pas in xp0.pas and xpconst.pas to remove some dependencies
    xpconst.pas should be used for global constants (only!)

  Revision 1.81  2003/12/13 23:06:50  cl
  - remove error message ''MAGGI.EXE fehlt'

  Revision 1.80  2003/11/22 15:32:22  mk
  - RasDial and RasHangup only for Win32

  Revision 1.79  2003/11/22 11:41:17  mk
  - added support for maximum mail size (pop3) and automatic dialing on win32

  Revision 1.78  2003/11/09 14:37:09  mk
  - Added debug log for MakeMimetypeCfg

  Revision 1.77  2003/10/29 11:18:08  cl
  - MausTausch I

  Revision 1.76  2003/10/20 21:23:35  cl
  - fixed DB access error before netcalls
  ...
  Revision 1.58  2002/08/03 16:31:41  mk
