...
{  $Id: xpmodemscripts.pas,v 1.14 2003/01/25 14:35:54 mk Exp $
...
{*    for i:=0 to MaxLines do script[i]:='';}
  ...
  procedure GetWord;     { n„chstes Wort lesen }
  var p : byte;
  begin
    stringflag:=false;
    if s='' then
...
begin    { of ExecuteScript }
  ip:=1;
  ReceivedChars:='';
  LoginTimer := TTimer.Create; LoginTimer.SetTimeout(boxpar^.LoginWait);
  UniTimer := TTimer.Create;
