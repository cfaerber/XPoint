{   $Id: xp2db.pas,v 1.59 2004/01/17 16:33:43 mk Exp $
...
{ XPOINT: Nachrichtendatei }
  ...
  MsgDbFields: array[0..MsgDbFieldCount] of dbFeldTyp = (
    ...
    (fname:'ServerUID'; ftyp:dbUntypedExt;  fsize:0; fnlen:0)
  );
  msg_msgid   = 15;
  msg_netztyp = 16;
  msg_name    = 17;
...
begin
  ...
  mb_serveruid  := dbGetFeldNr(mbase,'serveruid'); 
...
{ Feld 'Adresse' in Brettdatei einfuegen (ab 2.11) }
  ...
  begin
    ...
    moment;
    dbGoTop(bbase);     { Flags-Feld korrigieren }
...
begin
  ...
  //process records
    while not dbEOF(mbase) do begin
      ...
      if mimetypnr <> 0 then begin
        if hdp.mime.ContentType.AsString<>'' then
          dbWriteNStr(mbase, mimetypnr, hdp.mime.Contenttype.AsString);
        if hdp.boundary<>'' then begin
  ...
  if not FileExists(MsgFile+dbExtExt) then
    dbKillXbase(MsgFile);
end;
...
{$ELSE}
  ...
  if not FileExists(DupeFile+dbExtExt) then
    dbKillXbase(DupeFile);
{$ENDIF}
...
{
  $Log: xp2db.pas,v $
  Revision 1.59  2004/01/17 16:33:43  mk
  - split xp0.pas in xp0.pas and xpconst.pas to remove some dependencies
    xpconst.pas should be used for global constants (only!)

  Revision 1.58  2003/11/05 21:21:15  mk
  - fixed false closing of bbase

  Revision 1.57  2003/10/21 21:43:17  cl
