{   $Id: montage.pas,v 1.12.2.1 2002/07/21 20:14:33 ma Exp $
...
const
       monat : Array[1..12] of record
               ...
               ((tag: 'Januar';   zahl: 31), (tag: 'Februar';  zahl: 28),
                (tag: 'MÑrz';     zahl: 31), (tag: 'April';    zahl: 30),
...
begin
  with dd do
    if t+m+j > 0 then { MK 01/00 falls Record leer, wird 1 (Mo) zurÅckgegeben }
    begin
      if m<3 then begin
        m:=m+12; dec(j);
      end;
...
{ letzter Sonntag im MÑrz - letzter Sonntag im September }
...
begin
  ...
  if ((m>3) and (m<10)) or                    { April bis September                       }
     ((m=3) and (t>29-d1)) or                 { Letzter Sonntag im MÑrz schon vorbei?     }
     ((m=3) and (t=29-d1) and                 { Letzter Sonntag im MÑrz nach 01:59 Uhr?   }
