...
procedure NeuMessage(VAR noeasy:boolean);
procedure Easy;
...
procedure NeuMessage(VAR noeasy:boolean);
  ...
  begin
    ...
    mon;
    pushhp(1550);
    noeasy := (ReadButton(x+35,y+msglines+5,2,'*'+getres2(14000,30),1,true,z) <> 1);
    pophp;
    closebox;
    freeres;
  end;
  ...
  begin
    ...
    dialog(ival(getres2(252,100)),9+msglines,getres2(252,101),x,y);  { 'Adreáeinstellungen (ZCONNECT / RFC)' }
    msglines:=ival(getres2(14001,0));
    for i:=1 to msglines do
    begin
      maddtext(3,i+1,getres2(14001,i),col.coldialog);
    end;
    maddstring(3,3+msglines,getres2(252,102),orga^,47,OrgLen,'');    { 'Organisation  ' }
    mhnr(1040);
    maddstring(3,4+msglines,getres2(252,103),postadresse^,47,PostadrLen,'');   { 'Postanschrift ' }
    msetvfunc(TestPostanschrift);
    maddstring(3,5+msglines,getres2(252,104),telefonnr^,47,TeleLen,'>VFBQP +-0123456789');
    msetvfunc(TestTelefon);                                 { 'Telefon       ' }
    maddstring(3,6+msglines,getres2(252,105),wwwHomepage^,47,Homepagelen,range(' ','~'));
    msetvfunc(TestUrl);
    maddbool(3,8+msglines,getres2(252,109),adrpmonly);   { 'Adresse, Telefon und Homepage nur in PMs' }
    readmask(brk);
    closemask;
    closebox;
