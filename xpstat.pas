...
begin
  ...
  smax:=min(sysmax, smax); { MK 01/00 Nur bis Anzahl der gewÑhlten Systeme }
  ...
  if brk then
    ...
    if marked then
      if bmarkanz>1 then writeln(t,getreps2(2604,4,strs(bmarkanz)))   { 'fÅr %s gewÑhlte Bretter' }
        ...
        writeln(t);
        writeln(t,getreps2(2604,5,mid(dbReadStr(bbase,'brettname'),2)));   { 'fÅr %s' }
        end
    else
...
{
  $Log$
  Revision 1.5.2.2  2003/01/15 16:02:19  mw
