...
const
  maxrec = 4500; { MK 01/00 von 3500 auf 4500 erhîht }
  ...
  if monate then
    for i:=firstmonth to lastmonth do begin
      ...
      if maxgeb=0 then
        ...
        writeln(t,'ƒƒƒƒƒƒ'+dup(length(waehrung)+1,'ƒ')+'≈ƒ'+dup(tage*2,'ƒ'));
...
{
  $Log$
  Revision 1.14.2.9  2003/05/01 14:23:01  mk
  ...
  MW: Noch eine kleine ErgÑnzung zum GebÅhrenupdate:
      - Jetzt auch WÑhrung im Balkendiagramm der GebÅhrenstatistik
  ...
  MW: Kleine ErgÑnzungen zum gro·en GebÅhrenupdate :
      ...
      - Default-Tarif in der Boxenconfig ist jetzt
        "DTAG T-NET City"
      - Die GebÅhrenstatistik unter X/S/E zeigt in
