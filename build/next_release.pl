...
open OLD, "<build/openxp.spec";
open NEW, ">build/openxp.spec.new";

while(<OLD>) {
  if(m/^Version:/) {
    printf NEW "Version: %s\n", $version;
    printf NEW "Release: 1\n";
    printf NEW "Source: openxp-%s\n", $version;
  }
  elsif(m/^Release:/) { }
  elsif(m/^Source:/) { }
  elsif(m/^%changelog/) {
...
rename "build/openxp.spec.new", "build/openxp.spec";
rename "xpdefine.inc.new",	"xpdefine.inc";

system 'cvs', 'commit', '-m', 'New version number: '.$version,
  "xpglobal.pas", "debian/changelog", "build/openxp.spec", "xpdefine.inc" || die;
