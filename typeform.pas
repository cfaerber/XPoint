{   $Id: typeform.pas,v 1.146 2003/12/08 10:35:07 mk Exp $
...
function Coalesce(const s1, s2: string):string; overload; { ersten String <> '' zur¸ckliefern }
function Coalesce(const s1, s2, s3: string):string; overload; { ersten String <> '' zur¸ckliefern }
...
procedure Trim0(var s: String); { String am ersten #0 k¸rzen }
{$ifndef FPC}
...
function Coalesce(const s1, s2: string):string; { ersten String <> '' zur¸ckliefern }
begin
  if s1 <> '' then
    result := s1
  else
...
function Coalesce(const s1, s2, s3: string):string; { ersten String <> '' zur¸ckliefern }
begin
  if s1 <> '' then
    result := s1
  else
...
begin
  s2:=s;
  conv('Ñ','a');  //todo: use StringReplace
  conv('î','o');
  conv('Å','u');
  conv('·','s');
  conv('é','A');
...
{ Sucht nach char in Buffer mit L‰nge Len }
