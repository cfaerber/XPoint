{   $Id: zcrfc.pas,v 1.148 2004/01/17 16:33:50 mk Exp $
...
//        else
  ...
  begin
    ...
    if UpperCase(wab) = UpperCase(absender) then
      wab := '';
    betreff     := RFC2047_DecodeOld(betreff,csCP437);
//  realname    := RFC2047_Decode(realname,csCP437);
    summary     := RFC2047_DecodeOld(summary,csCP437);
    keywords    := RFC2047_DecodeOld(keywords,csCP437);
    organisation:= RFC2047_DecodeOld(organisation,csCP437);
    postanschrift:=RFC2047_DecodeOld(postanschrift,csCP437);
    fido_to     := RFC2047_DecodeOld(fido_to,csCP437);

    for i := 0 to ULine.Count-1 do
    begin
      s := ULine[i];
      s := RFC2047_DecodeOld(s,csCP437);
      ULine[i] := s;
    end;
