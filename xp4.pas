{   $Id: xp4.pas,v 1.149 2004/01/18 19:58:38 mk Exp $
...
begin
  ...
  begin
    ...
    showscreen(false);
    rec:=dbRecno(mbase);
    if netcall2(true,node,false,false,true) then;
    dbGo(mbase,rec);
    m2t:=false;
    holen(p);
  end;
...
//  setall;
  ...
  begin
    if (disprec[1]>0) then begin
      ...
      if dbReadInt(mbase,'gelesen')<>0 then begin
        p:=2;
        while (p<maxgl) and (((disprec[p]<>0) and ((length(dispbuf[p])>1) and (dispbuf[p][2]<>'>')))
  ...
  begin
    if AutoCrash<>'' then begin
      ...
      if netcall2(true,AutoCrash,false,false,crash) then;
      AutoCrash:='';
      SetAll;
      end;
  end;
