...
{ CrossPoint - Hauptmenü }

{ variable Menüpunkte setzen }
...
begin
  ...
  dialog(30,4,'Bildschirmgröße einstellen',x,y);     // 'Bildschirmgröße einstellen'
  ...
  begin
    ...
    ScreenLines := aLines;
    ScreenWidth := aCols;
    // Diese Änderungen speichern wir auch in der Config ab
...
begin
  menurestart:=false;
  repeat
   if (t=keyf4) and (lastmenusel>0) and       { v- Menüpunkt nicht disabled }
...
{$ENDIF CAPI }
     ...
     22 : if not (dispmode in [10..19]) then
            rfehler(442)    { 'Nur in der Nachrichtenübersicht möglich.' }
          else
            case m1 of           { Ändern }
              1 : ModiBetreff;
              2 : ModiEmpfdatum;
     ...
     24 : case m1 of               { Extern }
            ...
            5 : pmcOptions;
            6 : PGP_Options;
          { 7 : ViewerOptions;   - nicht mehr verwenden: könnte noch }
          {                        in keydef.cfg stehen              }
          end;

     26 : GebuehrenZaehler(m1=1);    { Zusatz/Gebühren }
     ...
     33 : case m1 of          { Wartung / Sonstiges }
            ...
            4 : bd_setzen(true);     { Datumsbezüge wiederherstellen }
          end;
    ...
    42 : begin                    { Spezielle Themen }
            case m1 of
              1 : do_help(1010);   { PGP-Verschlüsselung }
...
{    for x:=1 to keylen+2 do
        if (x>ks1) and (x-1<=ks2) then
          ...
          mem[base:ofs+2*x]:=c1;
       Hier wird von den Menüpunkten das Attribut geändert }
      mon;
      end; *)
  end;
