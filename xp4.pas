...
{$ENDIF }
  ...
  xp0,xp1,xp1help,xp1input, xpconst,
...
begin
  ...
  setenable(Suchen,6,Marked.Count<>0);
  setenable(Suchen,7,Marked.Count<>0);
  ...
  repeat
   ...
   {  8 in Xp1Menu.inc/Menu_keep  ( Edit/Schablonen )}
    ...
    { 13 in xp1menu.inc/Menu_keep    (Extrakt/Als...) }
     ...
     16 : case m1 of                                  { Edit }
            2: UniSel(usBoxes, true, '');
            3: UniSel(usGroups, true, '');
            4: UniSel(usSystems, true, '');
            5: UniSel(usShortNames, true, '');
            6    : begin
                     select(20);    { AutoVersand }
                     ShowMain(0);
                   end;
            7    : EditText;
            8    : if UniSel(usMIMETypes,true,'')='' then;
          end;
...
{$ENDIF CAPI }
     ...
     21 : case m1 of           { Weiterleiten }
            1     : Weiterleit(wlKopie,true);       { Kopie     }
            2     : Weiterleit(wlOriginal,true);    { Original  }
            3     : Weiterleit(wlEditTo,true);      { EditTo    }
            4     : Weiterleit(wlQuoteTO,true);     { QuoteTo   }
            6     : empfang_bestaetigen(nix);
            7     : CancelMessage;
            8     : ErsetzeMessage;
            9     : Weiterleit(wlErneut,true);      { Erneut    }
           10     : Weiterleit(wlArchiv,true);      { Archiv    }
           11     : pm_archiv(false);
         end;
...
//      sData.Org_Ref := orghdp.org_msgid;
        ...
        // -- Gequotete Nachricht hinzuf¸gen ---------------------------
        ...
        begin
          ...
          begin 
            ...
            if (qMimePart = nil) and (Lowercase(Orghdp.mime.contenttype.verb) <> 'text/plain') then
            begin
              pushhp(94);
              try
...
begin      { --- select --- }
{$IFDEF Debug }
 //FIXME://FIXME://FIXME://FIXME://FIXME://FIXME://FIXME: dbLog('--- select('+strs(dispmode)+')');
{$ENDIF }
...
{      rfehler(414); }  { 'markier-Anzeige ist bereits aktiv' }
  ...
  repeat
    ...
    if (dispmode<>3) and (dispmode<>4) and (dispmode<>-1) and
      ...
      {$IFDEF Debug }
        //FIXME: dbLog('--- select, t:<'+t+'>, dispmode: '+strs(dispmode));
      {$ENDIF }
...

{
  ...
  - fixed #704019: Empf‰nger "Alle" bei Fido-Antworten
