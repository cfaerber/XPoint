...
type
      ...
      absatzt  = packed record
                   next,prev  : absatzp;
                   size,msize : Integer16;       { msize = allokierte Grî·e }
                   umbruch    : boolean;
...
{ Offset der Cursorposition in dl^[scy].absatz; kann grˆﬂer }
{ als die L‰nge des Absatzes sein!                          }
...
var b1l,b2l : integer;        { vorauss.: disp[x]=2 f¸r mind. ein x }
begin
  with e^ do
...
{ Korrektur nach Lˆschen innerhalb eines Absatzes }
...
{ Korrektur nach Einf¸gen innerhalb eines Absatzes }
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ap:=dl^[1].absatz; l:=dl^[1].zeile-1;    { Abs. r¸ckw‰rts suchen }
...
procedure NewLine;              { Enter - Absatz einf¸gen }
var ap,ap2   : absatzp;
    wp       : integer;
    delsp    : integer;
    spaces   : integer;
...
procedure DELchar;              { DEL - Zeichen lˆschen }
var ap: absatzp;
    wp  : word;
    addspaces    : integer;
    wpa          : absatzp;
  ...
  with e^ do begin
    ...
    if wp<ap^.size then
      ...
      if ap^.next=nil then    { Textende }
        ...
        Debug.Debuglog('editor.inc','Entering debugging for #1327796',dlDebug);    
...
procedure BackSpace;            { Backspace - Zeichen lˆschen }
begin
  if EndSpaces>0 then
...
procedure WortRechtsLoeschen;   { Wort rechts lˆschen }
var 
...
procedure WortLinksLoeschen;    { Wort links lˆschen }
var wp,wpnew: word;
    wpa             : absatzp;
begin
  if workpos=0 then
...
procedure ZeileLoeschen;        { akt. Zeile lˆschen }
var apd    : absatzp;
    bc     : boolean;
begin
  with e^ do                         { 1. Fall: letzte Zeile im Text }
    if (ActAbs^.next=nil) and (dl^[scy].zeile=alines(ActAbs)) then begin
    ...
    else if alines(ActAbs)=1 then begin   { 2. Fall: kompletten Absatz lˆschen }
      ...
      ActAbs^.next:=apd; ActAbs^.prev:=nil;  { leeren Absatz anh‰ngen }
      apd^.prev:=ActAbs;
      AddDelEntry(ActAbs);
      KorrBlockScrolled(true);
    ...
    else                     { 3. Fall: Zeile aus Absatz lˆschen }
      AbsDelete(ActAbs,dl^[scy].offset,LineLength,true,true);
  aufbau:=true;
end;
...
begin
  with e^ do begin
    ...
    if not insertmode and (wp<ap^.size) then begin   { Overwrite }
      ...
      begin
          Debug.Debuglog('editor.inc','Debug for #1327796, No. 2 ',dlDebug);       
          Debug.Debuglog('editor.inc','ap^.size: '+IntToStr (ap^.size),dlDebug);   
...
{ Block an Cursorposition einf¸gen; Blockzeiger auf Ende zur¸ckliefern }
...
begin
  ...
  spaces:=EndSpaces;
  wp:=workpos;
  if blk^.next=nil then begin        { Absatzausschnitt einf¸gen }
    ss:=ap^.size+blk^.size+spaces;
    Debug.Debuglog('editor.inc','ss: '+IntToStr (ss),dlDebug);                     
    Debug.Debuglog('editor.inc','maxabslen: '+IntToStr (maxabslen),dlDebug);
    if ss>maxabslen then
    begin
      Debug.Debuglog('editor.inc','ss>maxabslen, Error',dlDebug);
      error(2)     { 'Absatz zu groﬂ }
    end  
  ...
  else begin                              { mehrere Abs‰tze einf¸gen }
    apn:=blk;
    ...
    if (ap^.size+blk^.size>maxabslen) or
      ...
      Debug.Debuglog('editor.inc','ap^.size+blk^.size>maxabslen, Error'+IntToStr(ap^.size)+'+'+IntToStr(blk^.size)+'>'+IntToStr(maxabslen),dlDebug);
      error(2);     { 'Absatz zu groﬂ }
      FreeBlock(blk);
    end
      ...
      ss:=ap^.size+spaces-wp;              { Grˆﬂe 2. Absatzteil }
...
begin
  with e^ do begin
    ...
    if blockinverse or blockhidden then begin
      ...
      error(2)                         { mehrere Abs‰tze ausschneiden }
    else begin
      recount:=(block[1].disp<2);
      wp.absatz:=ActAbs;
      ss:=ap1^.size-ofs1;
      ...
      copyflags(ap1,apnew);
      Move(ap1^.cont[ofs1],apnew^.cont,ss);
      if ap1^.next<>ap2 then begin     { dazwischenliegende Abs‰tze }
        apnew^.next:=ap1^.next;   { anh‰ngen                   }
      ...
      apl^.next:=apn;             { .. an Cut-Block anh‰ngen }
      apn^.prev:=apl;
      ...
      if ap=nil then
        ...
        if ersetzen then with language^ do begin
          ...
          if t[1]=replacechr[1] then begin  { ersetzen: Ja }
            insap:=AllocAbsatz(length(repby));  { Einf¸geabsatz erzeugen }
