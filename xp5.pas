...
{$IFNDEF ver32}
        FastMove(mem[base:0],mem[base:160],(vlines-1)*160);
{$ENDIF}
...
{$IFNDEF ver32}
        FastMove(p^,mem[base:i*160],(vlines-i)*160);
{$ENDIF}
        delay(5);
        end
    else
{$IFNDEF ver32}
      FastMove(p^,mem[base:0],vlines*160);
{$ENDIF}
  end;
...
begin
  ...
  repeat
    ...
    moff;
{$IFNDEF ver32}
    FastMove(mem[base:0],p^,vrows2*vlines);
    mattr:=textattr;
