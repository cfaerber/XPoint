{   $Id: xpsendmessage_attach.pas,v 1.31 2004/01/18 14:58:16 mk Exp $
...
{$ENDIF}
  ...
  xpsendmessage_attach_analyze, xpstreams, xp_pgp, xpconst,
  xpcharset,
  xpcharset_streams,
  xpglobal;
...
procedure SendAttach(parts:TList;const Signature:string;Netztyp:eNetz);
  ...
  {$IFDEF Win32}
    ...
    begin
      ...
      begin
        Windows.FileTimeToLocalFileTime(utc,{$IFDEF FPC1}@{$ENDIF}local);
        Windows.FileTimeToSystemTime(local,{$IFDEF FPC1}@{$ENDIF}wsyst);
      {$IFDEF FPC}
...
begin
  ...
  begin
    ...
    TED(FileName,true,iif(editvollbild,0,2),false,true,false);
...
{
  $Log: xpsendmessage_attach.pas,v $
  Revision 1.31  2004/01/18 14:58:16  mk
  - fixed 865721: Voreinstellung beim MIME-Anhang

  Revision 1.30  2004/01/17 16:33:49  mk
  - split xp0.pas in xp0.pas and xpconst.pas to remove some dependencies
    xpconst.pas should be used for global constants (only!)

  Revision 1.29  2003/12/13 22:48:37  cl
  - BUGFIX: initialise disposition type for attachment
