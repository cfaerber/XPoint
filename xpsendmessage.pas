{ $Id: xpsendmessage.pas,v 1.84 2004/01/17 16:33:49 mk Exp $
...
begin
  ...
  if UpperCase(orgHdp.MIME.ContentType.MainType) = 'MULTIPART' then
  begin
    PartFile := fn;
    PartFTmp := temp;
...
begin
  ...
  begin
    ...
    pa.ContentDisposition.AsString := iifs(OrgHdp.Mime.Disposition.AsString<>'',OrgHdp.Mime.Disposition.AsString,'inline');
    pa.ContentDescription := OrgHdp.Mime.Description;
    pa.ContentType.AsString := iifs(OrgHdp.Mime.contenttype.asstring<>'',OrgHdp.Mime.contenttype.asstring,'text/plain');
...
begin
  if PartsEx or not assigned(OrgHdp) then
    ...
    result := UpperCase(OrgHdp.MIME.ContentType.MainType)='MULTIPART';
end;
...
{
  $Log: xpsendmessage.pas,v $
  Revision 1.84  2004/01/17 16:33:49  mk
  - split xp0.pas in xp0.pas and xpconst.pas to remove some dependencies
    xpconst.pas should be used for global constants (only!)

  Revision 1.83  2003/10/29 11:18:08  cl
  - MausTausch I

  Revision 1.82  2003/10/21 21:25:04  cl
