...
procedure AppLog(var logfile:string; const dest:pathstr);   { Log an Fido/UUCP-Gesamtlog anhÑngen }
...
{
  $Log$
  Revision 1.13.2.34  2003/05/01 14:22:53  mk
  ...
  MY:- Unversandt-Routine ('ClearUnversandt') optimiert:
       ...
       2. Bei Crosspostings und Mails mit KopienempfÑngern vom Netztyp
          RFC/Client wird jetzt nur noch beim Durchlauf fÅr den ersten
          EmpfÑnger in der Textdatei "UNSENT.ID" nach der Message-ID
          gesucht und das Ergebnis (Nachricht ist versandt bzw.
          unversandt) in einer Variablen gesichert. Bei den
          anschlie·enden DurchlÑufen wird auf diese Variable
          zurÅckgegriffen statt jedesmal erneut die Datei zu îffnen und
          auszulesen.
       ...
       Es handelt sich speziell bei den ersten beiden énderungen *nicht*
       um Bugfixes - die Routine arbeitet jetzt lediglich in solchen FÑllen
