{   $Id: xp1o.pas,v 1.126 2004/01/17 16:33:42 mk Exp $
...
var flags,len: longint;
    ...
    fn: string;  
begin
  flags := dbReadIntN(mbase,mb_flags);

  if(flags and 65536) <> 0 then begin
    len := dbXSize(mbase,'serveruid');
    SetLength(id, len);
    if len <= 5 then exit; // broken id
    dbReadX(mbase,'serveruid',len,id[1]);
    if id[5]<>'P' then exit; // no POP3 ID
    server := Longint(LittleEndianToHost32(PDWord(@(id[1]))^));

    if dbSafeGo(boxbase, server) then
    try
      fn := dbReadStr(boxbase,'dateiname')+extUdd; 
      mode := fmOpenWrite; if not FileExists(fn) then mode := fmCreate;
      str := TFileStream.Create(dbReadStr(boxbase,'dateiname')+extUdd, mode or fmShareDenyWrite);
      try
        str.Seek(0,soFromEnd);
        writeln_s(str,Mid(id,6));
      finally
        str.Free;
      end;
    except;
...
{
  $Log: xp1o.pas,v $
  Revision 1.126  2004/01/17 16:33:42  mk
  ...
  Revision 1.112  2002/07/09 13:37:19  mk
