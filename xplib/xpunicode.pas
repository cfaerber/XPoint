...
type UTF8String = {$IFDEF Delphi} System.UTF8String; {$ELSE} AnsiString; {$ENDIF}
...
function UTF8Char(UChar: TUnicodeChar): UTF8String;

{ Creates a plane 14 tag from ASCII. }
function UTF8Tag(const tag: string): UTF8String;
...
function UTF8Char(UChar: TUnicodeChar): UTF8String;
begin
  if UChar <= $7F then result := Chr(UChar) else
  if UChar <= $7FF then result := Chr($C0 or (UChar shr 6)) + Chr($80 or (UChar and $3F)) else
  if UChar <= $FFFF then result := Chr($E0 or (UChar shr 12)) + Chr($80 or ((UChar shr 6) and $3F)) + Chr($80 or (UChar and $3F)) else
  if UChar <= $10FFFF then result := Chr($F0 or (UChar shr 18)) + Chr($80 or ((UChar shr 12) and $3F)) + Chr($80 or ((UChar shr 6) and $3F)) + Chr($80 or (UChar and $3F)) else
  result := UTF8Char($FFFD);
end;
...
begin
  ...
  for i := 1 to Length(tag) do
    if tag[i] in [#$20..#$7E] then
      result := result + UTF8Char(TUnicodeChar($E0000 + Ord(tag[i])));
end;
...
function UTF16Char(UChar: TUnicodeChar): UTF16String;
begin
  if UChar <= $FFFF then begin
    SetLength(result,1);
    result[1] := WideChar(UChar);
  end else
  begin
    SetLength(result,2);
    UChar := UChar - $10000;
    result[1] := WideChar($D800 or (UChar shr 10));
    result[2] := WideChar($DC00 or (UChar and $3FF));
  end;
end;
{$ENDIF}
