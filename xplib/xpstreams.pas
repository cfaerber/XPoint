{
  $Id: xpstreams.pas,v 1.3 2004/01/30 11:51:54 cl Exp $
...
procedure CopyStream(InStream,OutStream:TStream);
begin
  CopyStreamMult(InStream,[OutStream]);
end;
...
begin
  ...
  begin
    for i:= Low(OutStreams) to High(OutStreams) do
      OutStreams[i].WriteBuffer(
        (PChar((InStream as TCustomMemoryStream).Memory) + InStream.Position)^,
        InStream.Size - InStream.Position);
  end else

  if InStream is TStringStream then
  begin
    for i:= Low(OutStreams) to High(OutStreams) do
      OutStreams[i].WriteBuffer(
        (InStream as TStringStream).DataString[InStream.Position+1],
        InStream.Size - InStream.Position);
  end else
