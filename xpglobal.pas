{  $Id: xpglobal.pas,v 1.91 2003/11/13 17:08:09 mk Exp $
...
{$IFDEF Snapshot}
  ...
  // use dummy file build/snapdate.inc if ./snapdate.inc is missing (should
  // only happen if we compile directly from CVS and w/o Makefile).

  {$IFDEF FPC}
  {$INCLUDEPATH .;build}
  {$ENDIF}
  {$INCLUDE snapdate.inc}

  verstr     = mainver + '-' + snapdate;
