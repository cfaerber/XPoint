...
begin
  ...
  for i:=0 to 3 do
    mappsel(true,getres2(108,i));    { 'AlleùZ-NetzùKeineùPMs' }
...
begin
  ...
  msgbox(69,10,getres2(253,5),x,y); { 'VORSICHT: M™GLICHE KOMPATIBILITŽTSPROBLEME' }
...
{$IFDEF unix}
  ...
  maddbool(3,8,getres2(255,15),Enable_UTF8);   { 'UTF-8 aktivieren' }
    mhnr(8077);
...
{$ELSE }
  ...
  maddbool(3,9,getres2(255,15),Enable_UTF8);   { 'UTF-8 aktivieren' }
    mhnr(8077);
  maddbool(3,10,getres2(255,9),ListHighlight);  { 'farbliche *Hervorhebungen*' }
    mhnr(237);
  maddbool(3,11,getres2(255,12),QuoteColors);   { 'verschiedenfarbige Quoteebenen' }
    mhnr(8060);
  maddbool(3,13,getres2(255,10),ListScroller); { 'Rollbalken bei Mausbedienung' }
    mhnr(238);
  maddbool(3,14,getres2(255,11),ListAutoScroll);  { 'automatisches Rollen am Bildrand' }
  maddbool(3,16,getres2(255,13),ListEndCR);    { 'Lister mit <Return> verlassen' }
    mhnr(8061);
{$ENDIF } { Linux }
  freeres;
  readmask(brk);
...
begin
  ...
  begin
    maddstring(3,2,'ARC ',UnARC,38,50,'');
      msetvfunc(testarc);
      mappsel(false,'pkxarc $ARCHIV $DATEIùpkunpak -e $ARCHIV $DATEIùarc e $ARCHIV $DATEIùarce $ARCHIV $DATEI');
    maddstring(3,4,'ARJ ',UnARJ,38,50,'');
      msetvfunc(testarc);
      mappsel(false,'arj e $ARCHIV $DATEI');
    maddstring(3,6,'LZH ',UnLZH,38,50,'');
      msetvfunc(testarc);
      mappsel(false,'lharc e $ARCHIV $DATEIùlha e $ARCHIV $DATEI');
    ...
    maddstring(3,14,'ZIP ',UnZIP,38,50,'');
      msetvfunc(testarc);
      mappsel(false,'pkunzip $ARCHIV $DATEIùunzip $ARCHIV $DATEI');
...
//    mset1func(testpgpexe);
  ...
  mappsel(false,'--rfc1991 --cipher-algo ideaù--compress-algo 1 --cipher-algo cast5');
...

{
  ...
  Revision 1.140  2003/08/26 04:58:14  mk
