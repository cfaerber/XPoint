...
begin
  ...
  begin
    ...
    mwrt(71,1,' ');  { Anzeige Nachrichtenweiterschalter lîschen }
    GlobalModified;
  end;
  enddialog;
  menurestart:=brk;
...
(*  maddreal(3,2,getres2(1023,2),r,8,2,0,99999);   { 'Kosten fÅr nicht zustandegekommene Verbindung        ' }
  ...
  maddstring(3,5,getres2(1023,3),waehrung,5,5,'');   { 'WÑhrung' }
    mhnr(973);
  maddbool(24,5,getres2(1023,6),newgeb);    { 'neues GebÅhrenmodell' }
    mhnr(974);
  readmask(brk);
  if not brk and mmodified then begin
...
{
  $Log$
  Revision 1.39.2.40  2003/05/01 14:22:42  mk
  ...
  MY:- Anzeige des mit <Ctrl-W> manipulierten Nachrichten-Weiterschalters
       zur besseren Unterscheidung nochmals geÑndert: Gro·es "W"
       (= Weiterschalter aktiviert) wird in der Farbe fÅr den MenÅtext,
       kleines "w" (= Weiterschalter deaktiviert) wird in der Farbe fÅr
       deaktivierten MenÅtext dargestellt. Bei BestÑtigung des Config-
       MenÅs C/O/B wird die Anzeige entfernt, da dann wieder die
       MenÅoptionen gelten.
