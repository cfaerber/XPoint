{   $Id: maggi.pas,v 1.39 2003/10/21 21:42:25 cl Exp $
...
begin
  ...
  writeln(UpperCase(infile),'  ØØ  ',UpperCase(outfile));
  writeln;
end;
      ...
      if ok then with hd do begin
        ...
        if (p1=0) and
          ...
          if tomaus and (betreff='<Maus-Command>') then
            ...
            if maus_msgid <> '' then
            begin
              if maus_reference <> '' then
                writeln(t,'-', maus_reference);
              if GetLastReference <> '' then
                writeln(t,'R',GetLastReference);
            end else
            if GetLastReference <> '' then
...
*)                
            if followupto <> '' then
              writeln(t,':Followup-To: ',mid(followupto,length(bretth)+1));
          end;
...
begin
  ...
  begin
    ...
    if not eof(t1) then begin
      ...
      repeat
        ...
        begin
          ...
          case c of
                  ...
                  begin
                    ...
                    if firstline then begin
                      ...
                      then begin
                        ...
                        FollowUpTo := bretth+trim(mid(s,14))
                      else if LeftStr(LowerCase(s),10)='reply-to: ' then
                        FReplyto := trim(mid(s,11))
            ...
            'M' : begin
                    mime.version :=GetToken(s,';');
                    UseMIME:=true;
                    if LowerCase(GetToken(s,':'))='content-type' then
                      mime.contenttype.AsString := s;
                  end;
      ...
      if org_msgid<>'' then begin
        hd.maus_msgid := hd.msgid;
        hd.maus_reference := hd.GetLastReference;
        hd.msgid := org_msgid;
        hd.References.Clear;
        hd.References.Add(org_xref);
      end;
      ...
      if msgid='#LOG' then
        ...
        if (msgid<>'') and (cpos('@',msgid)=0) then begin   { Info-File }
          Firstempfaenger:='/ØMausinfo';    { s. auch XP0.MausInfoBrett }
        ...
        if not killmsg then begin
          ...
          if Freplyto <> '' then wrs('ANTWORT-AN: '+freplyto);
          wrs('DISKUSSION-IN: '+followupto);
          if programm<>''  then wrs('Mailer: '+programm);
          if gate<>''      then wrs('Gate: '+gate);
          if distribution<>'' then wrs('U-Distribution: '+distribution);
          if MIME.version<>'' then wrs('U-MIME-Version: '+MIME.version);
          if MIME.contenttype.Asstring<>'' then wrs('U-Content-Type: '+MIME.contenttype.AsString);
...
{
  $Log: maggi.pas,v $
  Revision 1.39  2003/10/21 21:42:25  cl
  - fixes for maggi

  Revision 1.38  2003/10/21 14:15:08  mk
