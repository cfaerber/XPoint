...
const fsb_shadow : boolean = false;   { fsbox: Schatten                  }
      fsb_info   : boolean = false;   { fsbox: Dateigrî·e/Datum anzeigen }
...
var   fb     : pathstr;
 ...
 aue,ade,au,ad,ab :boolean;
  ...
  begin
    ...
    begin
      hinweis(getreps2(2800,15,strs(maxavail)));  { 'Speichermangel! Max. verfÅgbar: %s' }
      freeres;
      exit;
    end;
    AddFnItem:=true;
    ...
    if fsb_info then begin
      ...
      begin
        ...
        if drive in [1..7] then
          ...
          { 'Festplatte' / 'RAM-Disk' / 'Subst-Laufwerk' / 'GerÑtetreiber' }
      ...
      repeat
        ...
        if modify and (t=keyins) then begin
          ...
          if not brk then begin
            ...
            if inoutres<>0 then begin
              if ioresult=3 then
                pmsg(getres2(2800,16))    { 'ungÅltiger Name - Taste drÅcken' }
              else
                pmsg(getres2(2800,17));   { 'Anlegen nicht mîglich - Taste drÅcken' }
              errproc;
              get(t2,curoff);
              end
            else begin
        ...
        if modify and (t=keydel) then begin
          ...
          if ioresult<>0 then begin
            pmsg(getres2(2800,18));       { 'Lîschen nicht mîglich - Taste drÅcken' }
            errproc;
            get(t2,curoff);
            end
          else begin
