...
begin
  ...
  begin
    ...
    TestS := NewS; { copy_address verÑndert String, retten }
    form_address(TestS);
...
begin
  ...
  if not brk then
  begin
    if (OldAdr <> 0) and (adr=0) then { Adr geÑndert von ungleich 0 zu 0 }
     if not readJN(GetRes(2738),false) then
       Adr:= OldAdr; { doch nicht? Dann zurÅcksetzen }
...
begin
  ...
  if MarkUnversandt and (left(dbReadStrN(mbase,mb_brett),1)='U') then begin
    new(hdp);
...
begin
  modibrett2:=false;
  if left(dbReadStrN(bbase,bb_brettname),1)<'A' then begin
    rfehler(2712);
    exit;
    end;
  ...
  dbGo(bbase,rec);
  if not brk then begin
    if { (ustr(adresse)=ustr(mid(dbReadStrN(bbase,bb_brettname),2))) or }
...
begin
  ...
  if not brk then begin
    ...
    modin:=ustr(brett)<>ustr(dbReadStrN(bbase,bb_brettname));
    if modin then begin
      rec:=dbRecno(bbase);
      ...
      if ustr(s)='SYSOP' then
        ...
        if dbFound then begin
          ...
          if not dbEOF(ubase) and
             (ustr(s)=ustr(left(dbReadStrN(ubase,ub_username),length(s)))) then
...
{
  $Log$
  Revision 1.25.2.31  2003/05/01 14:22:47  mk
  ...
  JG:- Bei erstmaliger Benutzung des User-Anlegedialogs nach Programmstart
       ...
       <Enter> fÅhrte jede weitere Taste zum Absturz (Variable 'adp^' war
       nicht initialisiert).
  ...
  JG+MY:- User editieren: bei "Adre·buchgruppe" <F2>-Auswahl aus den
          aktuell bereits verwendeten Adre·buchgruppen mîglich.

  JG+MY:- Neue MenÅpunkte beim Editieren markierter User:
          PrioritÑt / EmpfangsbestÑtigung / Vertreteradresse / Lîschen
